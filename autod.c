...
gcc -Os -std=c11 -Wall -Wextra -DNO_SSL -DNO_CGI -DNO_FILES -DUSE_SDL2_GUI -DUSE_SDL2_TTF \
    autod.c parson.c civetweb.c autod_gui_sdl.c \
    -o autod -pthread \
...
#ifdef USE_SDL2_GUI
/* ---- GUI snapshot adapter (no SDL deps) ---- */
static inline int progress_pct(void); /* already declared above in your file */
...
static void gui_fill_snapshot(autod_gui_snapshot_t *s, void *user) {
    ...
    s->count = n;
    for (int i=0; i<n; i++) {
        strncpy(s->nodes[i].ip,      g_nodes[i].ip,      sizeof(s->nodes[i].ip)-1);
        strncpy(s->nodes[i].role,    g_nodes[i].role,    sizeof(s->nodes[i].role)-1);
        strncpy(s->nodes[i].device,  g_nodes[i].device,  sizeof(s->nodes[i].device)-1);
        strncpy(s->nodes[i].version, g_nodes[i].version, sizeof(s->nodes[i].version)-1);
    }
    pthread_mutex_unlock(&g_nodes_mx);
